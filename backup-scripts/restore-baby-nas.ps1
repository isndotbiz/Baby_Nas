...
param(
    ...
    [string]$RestorePath = "D:\workspace\baby_nas_RESTORE",
...
try {
    Write-ColorLog "=== Restic Restore Utility ===" -Color Cyan
    ...
    # Safety check
    if ($OverwriteSource) {
        Write-ColorLog "`nWARNING: You are about to overwrite D:\workspace\baby_nas!" -Color Red
        $confirm = Read-Host "Type 'YES' to confirm"
        if ($confirm -ne "YES") {
            Write-ColorLog "Restore cancelled." -Color Yellow
            exit 0
        }
        $RestorePath = "D:\workspace\baby_nas"
    } else {
